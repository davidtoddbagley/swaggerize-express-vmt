 ...
 * author: Todd Bagley
 */
function middleware_mediaType_server2client(produces, consumes) {
    return function (req, res, next) {
        const method = req.method && req.method.toLowerCase();
...
function expressroutes(router, options) {
    ...
    Object.keys(routesMethod).forEach(function (routePath){
        router.use(
            ...
            middleware_mediaType_server2client(routesProduces[routePath], routesConsumes[routePath])
        );
    });
}

 ...
 */
function middleware_buildNotAllowed(methods) {    
...
/**
 * Validates Media Type to be consumed by server
 * - format of body recieved from client
 * @param mediaTypes
 * 
 * author: Todd Bagley
 */
function middleware_mediaType_client2server(mediaTypes) {
    return function (req, res, next) {
...
/**
 * Validates Media Type to be returned by server
 * - format of response to client
 * @param mediaTypes
 * 
 * author: Todd Bagley
 */
function middleware_mediaType_server2client(mediaTypes) {
    return function (req, res, next) {
...
function expressroutes(router, options) {
    ...
    routes.forEach(function (route) {
        ...
        routesMethod[routePath].push(route.method && route.method.toLowerCase());

        routesConsumes[routePath] = routesConsumes[routePath] || [];
        if (route.consumes && route.consumes.length > 0) {
            routesConsumes[routePath].push(...route.consumes);
        }
        routesConsumes[routePath] = routesConsumes[routePath].filter((v, i, a) => a.indexOf(v) === i);
    
        routesProduces[routePath] = routesProduces[routePath] || [];
        if (route.produces && route.produces.length > 0) {
            routesProduces[routePath].push(...route.produces);
        }
        routesProduces[routePath] = routesProduces[routePath].filter((v, i, a) => a.indexOf(v) === i);
    });

    Object.keys(routesMethod).forEach(function (routePath){
        router.use(
            pathRegexp(routePath),
            middleware_buildNotAllowed(routesMethod[routePath]),
            middleware_mediaType_client2server(routesConsumes[routePath]),
            middleware_mediaType_server2client(routesProduces[routePath] && routesProduces[routePath].length > 0 ? routesProduces[routePath] : routesConsumes[routePath])
        );
    });
}
